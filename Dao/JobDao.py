...
class JobDao(BaseDao):
    ...
    def getJobCountByJobSalary(self, start, end):
        sql = "select count(*) as jobCount from t_job_data where 1=1"
        params = []
        sql += " and jobAverageSalary > %s and jobAverageSalary < %s"
        params.append(start)
        params.append(end)
    ...
    def addJob(self, data={}):
        ...
        params = [data['jobCompany'], data['jobName'], data['jobType'], data['jobCity'], data['jobHighSalary'],
                  data['jobLowSalary'], data['jobAverageSalary']]
        result = self.execute(sql, params)
        self.commit()
        return result
